...
`REQ-TRACE-001` 要求：含“必须、不得、禁止、不能”等规范词的正文必须位于一个已登记 contract 的 section 范围内；新增或移动规范条款时必须同步 `docs/requirements.yaml`。每个 `ROADMAP.yaml` task 还必须直接声明适用的 `contracts`；若确实不引入或实现规范行为，则声明 `contracts: [TEST-GATE-001]` 并在任务计划中解释其仅受通用原子执行与测试门禁约束。`docs/requirements.yaml` 的 `roadmap_tasks` 与 task 的 `contracts` 必须双向一致，不能只维护一侧。P0 bootstrap check 先验证已登记 section、task、test category、双向 contract reference 和 task coverage；P0-T2 起由仓库 validator 检查未知 contract ID、失效内部链接、孤立 roadmap task、依赖环、未被 contract section 覆盖的规范条款、没有直接 contract 绑定的 task，以及没有 roadmap/test 映射的 contract。CI 检查的是稳定 contract/section，而不是依赖自然语言逐句猜测条款身份。
...
6. 最近的包级 `AGENTS.md`；
7. 使用 VEM 修改页面时，再读取 `.agents/skills/visual-ui-edit/SKILL.md`。
...
路线图任务完成不自动授权继续建设下一阶段。P1 gate 必须通过独立 pilot task，在至少 3 个未参与实现的代表性 React + Vite 项目上记录安装、首次选择、正确 source candidate、首次编辑保留/撤销和卸载结果，并使用相同任务、固定计时起止点和相同成功定义记录 no-VEM baseline；P2 gate 必须记录 fresh Edge profile 的配对成功、失败与恢复；P3/P4 必须报告独立 holdout，而不只报告用于调参的 fixture。

为避免完成完整 P0/P1 基础设施后才发现核心价值不足，P0 在 source anchor、最小 selector/summary 与 fixture 可用后、coordinator/HMR verification 大量建设前，必须执行一次 `P0 value micro-pilot`。该检查使用预先登记的 3–5 个窄任务，对比“Codex 直接搜索源码”和“selection summary + source candidate”两条路径，只测目标绑定完整性、source top-1/top-3、错误定位数和定位耗时。它不是发布 gate，不得产生“更快/更准确”的产品百分比声明，但若全部任务没有可观察改善、出现错误 source 自动归因，或 setup 成本明显高于节省的定位时间，后续 topology/verification 投入必须暂停并先调整 selection-to-source 路径。micro-pilot 的任务、起止点、原始结果和继续/调整判据必须在运行前写入版本化 evidence plan；不得复用 P1/P3/P4 holdout 作为调参样本。
...
在创建 monorepo 之前必须完成一次不实现产品功能的 P0 execution preflight，并保存可复核证据。P0-T0 可以创建独立于 workspace package manager 的最小 `scripts/preflight/` shell/PowerShell probe、preflight ADR 和 evidence schema，但不能创建产品 package 或伪造后续 `pnpm edge:preflight:verify` 已存在。预检至少证明：

- 当前目录属于正式 Git worktree，或由 ADR 指定另一个可提交、可审计的 canonical repository；evidence plan、ADR 和结果能在观察结果前后形成可复核 commit，未初始化的散装目录不能作为通过状态；
- 选定 execution profile、进程 owner 和文件系统位置明确。P0 Tier-1 推荐并默认采用 `WSL Node/Vite/MCP + WSL ext4 project/runtime + Windows Edge Stable`；若从 WSL 在 `/mnt/*` DrvFS 项目目录运行 Node/Vite，必须单独通过 symlink、大小写、权限、pnpm install、文件监听和 HMR 延迟门禁，不能把 `/mnt/*` 的宽松 Linux mode 当作私有 ACL 证据；
- Node 使用受支持的 22.x LTS 基线且版本不低于 22.12，pnpm 通过 Corepack 或等价可复现方式固定 exact version；Git、PowerShell/WSL interop、package registry、代理/CA、磁盘空间、路径长度、大小写和 symlink 行为可用；P0-T1 只能把已证明的版本写入 workspace metadata，不能临时换用未预检工具链；
- Windows Edge Stable 可由 Playwright `channel: msedge` 或 ADR 选定的同等方式真实启动，记录 Edge/Windows/Playwright channel 与 headed/headless 结果，并能区分缺少 Edge、企业策略和自动化 channel 失败；只发现 `msedge.exe` 不算通过；
...
P0-T0 的规范产物固定为 `docs/adr/0001-edge-execution-preflight.md`、`docs/test-evidence/P0-T0/<run-id>/environment.json`、对应的人类可读摘要和 `scripts/preflight/` probes；JSON 至少记录命令、版本、execution profile、文件系统类型、runner owner、时间、结果和 artifact hash，不记录 token 或环境 secret。若发布 gate 依赖手工或 self-hosted Windows runner，runner owner、触发方式、证据保存位置和不可在普通 Linux CI 伪造 `passed` 的规则必须在 ADR 中锁定。预检失败时 P0-T1 及其依赖工作保持 blocked，避免在最后一个 E2E task 才发现 Tier-1 环境或工具链不可用。
...
- P0-T9 必须从两个真实 OS process 启动 fixture Vite 与 MCP STDIO adapter，不能只用同进程测试替代；
...
| 等级 | 首次阶段 | 内容 | 重启语义 |
|---|---|---|---|
| ephemeral browser/session state | P0/P2 | handles、token、pending metadata | document/browser/project 结束即失效 |
...
- `@vitejs/plugin-react` 或选定 React plugin 的版本与 Babel/SWC 路径；
...
`handleHotUpdate` 只表示服务器处理更新，不表示 Edge 已完成 React commit 和布局。每次更新必须保留因果范围：
...
1. P0：先完成 Git/toolchain/filesystem/Edge/Windows/WSL/ACL/loopback/WebSocket/HMR/双进程 execution preflight，再建立最小项目骨架；在 selector、anchor 和 fixture 可用后先运行 3–5 项预登记 value micro-pilot，再继续 topology 与 verification 投入；将 transform 与 registry、topology、proxy/build channel、MCP surface、verification prepare、update matching 和 complete 拆成可独立失败的原子任务，完成真实双进程 Edge Vite 注入式 walking skeleton（P0 reference topology→最小隐私与 selection provenance→选择→anchor→最小 EvidenceGraph/source resolution→value micro-pilot→prepare barrier→相关 HMR batch journal replay→文本验证→生产无泄漏）；
...
36. P0-T0 在 canonical Git worktree、固定 Node/pnpm、选定文件系统、真实 `msedge` launch、HTTP/WS/HMR、双进程、loopback 暴露和 private runtime ACL 任一项未通过时保持 blocked；只找到可执行文件或只跑 Linux Chromium 不能冒充 Tier-1 passed。
37. P0 value micro-pilot 在 topology/verification 大量建设前完成 3–5 个预登记定位任务；错误 source attribution 或无可观察价值改善会暂停后续投入，且该样本不得污染后续 holdout。
38. roadmap task 的直接 `contracts` 与 `docs/requirements.yaml` 反向映射双向一致；缺失绑定、未知 contract 或只修改一侧都使 validator 失败。

# 附录 B：调研与实现参考

以下资料用于学习产品模式、协议或平台能力，不代表可以复制其代码。最近复核日期为 2026-07-12；P0-T2 起 blocking CI 检查仓库内部链接与 requirements section，外部 URL 采用可重试的定期 reference audit 并记录最后成功日期，不能因临时网络故障阻塞每次本地提交。采用具体实现前还必须在 ADR 中记录所参考的 release/tag/commit，避免随网页更新而改变设计依据：
...
- Vite 当前 Node.js compatibility note：<https://vite.dev/guide/>

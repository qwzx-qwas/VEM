...
import {
  executeR5BoundedProcess as executeR6BoundedProcess,
  installR5SignalForwarding as installR6SignalForwarding,
  verifyR5EvidenceManifest as verifyR6EvidenceManifest,
...
const PREREGISTRATION_PARENT = join(REPO_ROOT, "docs/test-evidence/R6-T3");
const RESULT_PARENT = join(REPO_ROOT, "docs/test-evidence/R6-T4");
...
export const R6_RUNTIME_SOURCE_PATHS = Object.freeze([
...
export function verifyR6OwnerAuthorization({
  authorization,
  preregistrationHash,
  dataScopeHash,
  terminationPolicy,
  ...
  if (!HASH.test(preregistrationHash)
    ...
    || !HASH.test(outerEnvironmentHash)
    || !isRecord(authorization)
    || authorization.schemaVersion !== "R6-T4-owner-authorization-v1"
    || authorization.taskId !== "R6-T4"
    || authorization.decisionKey !== "R6-RECOVERY"
    || authorization.authorized !== true
    || authorization.preregistrationHash !== preregistrationHash
    || authorization.destination !== R6_DESTINATION
    || authorization.model !== R6_MODEL
    ...
    || authorization.successfulArmCount
      !== R6_TERMINATION_POLICY.requiredSuccessfulArms
    || authorization.maxProcessAttempts
      !== R6_TERMINATION_POLICY.maxProcessAttempts
    || canonicalJson(authorization.terminationPolicy)
      !== canonicalJson(terminationPolicy)
    || canonicalJson(terminationPolicy)
      !== canonicalJson(R6_TERMINATION_POLICY)
    ...
    throw new Error("R6_T4_OWNER_AUTHORIZATION_INVALID");
  }
  return Object.freeze({
    valid: true,
    maxProcessAttempts: R6_TERMINATION_POLICY.maxProcessAttempts,
    deadlineMs: R6_TERMINATION_POLICY.deadlineMs,
    graceMs: R6_TERMINATION_POLICY.graceMs,
    gracefulSignal: R6_TERMINATION_POLICY.gracefulSignal,
    forceSignal: R6_TERMINATION_POLICY.forceSignal,
    authorizationHash: canonicalSha256(authorization),
  });
}

export function verifyR6BoundSources(
  sourceBindings,
  { repoRoot = REPO_ROOT } = {},
) {
  const paths = Object.keys(sourceBindings ?? {}).sort();
  const expected = [...R6_RUNTIME_SOURCE_PATHS].sort();
  if (canonicalJson(paths) !== canonicalJson(expected)) {
    throw new Error("R6_T4_SOURCE_BINDINGS_INVALID");
  }
  for (const path of expected) {
    const hash = sourceBindings[path];
    if (!HASH.test(hash)
      || sha256(readRegular(resolve(repoRoot, path))) !== hash) {
      throw new Error("R6_T4_BOUND_SOURCE_CHANGED");
    }
  }
  return Object.freeze({
    valid: true,
...
export function verifyR6Preregistration(
  rootPath,
  { repoRoot = REPO_ROOT, requireProbes = true } = {},
) {
  const root = requireDirectory(rootPath, "R6_T4_PREREGISTRATION_INVALID");
  ...
  if (!HASH.test(expected) || actual !== expected) {
    throw new Error("R6_T4_PREREGISTRATION_CHANGED");
  }
  ...
  const bindings = verifyR6BoundSources(plan.sourceBindings, { repoRoot });
  if (plan.schemaVersion !== "R6-T3-preregistered-recovery-plan-v1"
    ...
    || plan.model !== R6_MODEL
    || plan.destination !== R6_DESTINATION
    ...
    || plan.terminalInputs?.r5?.phase !== "R5"
    || plan.terminalInputs?.r5?.status !== "failed"
    || plan.terminalInputs?.r5?.verdict !== "stop"
    ...
    || plan.compatibilityPreflight?.predicatePassed !== true
    || plan.compatibilityPreflight?.modelCall !== false
    || plan.compatibilityPreflight?.providerNetworkProbed !== false
    || canonicalJson(terminationPolicy)
      !== canonicalJson(R6_TERMINATION_POLICY)
    ...
    || canonicalSha256(R6_VERDICT_RULE) !== plan.verdictRuleHash
    || manifest.schemaVersion !== "R6-T3-task-manifest-v1"
    ...
    throw new Error("R6_T4_PREREGISTRATION_INVALID");
  }
...
export async function runR6RecoveryPilot({
  preregistrationRoot,
  resultRoot,
  authorizationPath,
  ...
  const verified = verifyR6Preregistration(preregRoot);
  const authorization = readJson(resolve(authorizationPath));
  const authorizationEvidence = verifyR6OwnerAuthorization({
    authorization,
  ...
  if (typeof executeAttempt !== "function") {
    throw new Error("R6_T4_EXECUTOR_INVALID");
  }
  ...
  const removeSignalForwarding = installR6SignalForwarding({
    controller,
  ...
  const verdict = evaluateR6Recovery({
    arms,
    processAttempts,
  ...
  writeJson(join(outputRoot, "results/run-index.json"), {
    schemaVersion: "R6-T4-run-index-v1",
    preregistrationHash: verified.preregistrationHash,
    instrumentationHash: verified.plan.instrumentationHash,
  ...
  return Object.freeze({
    ...
    remainingProcessAttemptBudget:
      R6_TERMINATION_POLICY.maxProcessAttempts - processAttempts.length,
...
async function executeDefaultAttempt({
  ...
  const recorded = await executeR6BoundedProcess({
    invocation,
    outputRoot: runRoot,
    runId,
    ...
    evaluate({ finalResponse }) {
      ...
      const parsed = validateR6ResponseText(finalResponse);
      if (parsed === null) {
        return {
          status: "failed",
          valid: false,
          ...
          failureCodes: ["R6_AUTHORITATIVE_RESPONSE_INVALID"],
        };
      }
      ...
      return {
        ...
        failureCodes: matches ? [] : ["R6_WRONG_ATTRIBUTION"],
      };
    },
    signal,
  });
  ...
  const classification = classifyR6Attempt({
    ...
    evidenceSealed: recorded.evidenceSealed
      && verifyR6EvidenceManifest(runRoot),
  });
...
function runCurrentPreflight(preregRoot, manifest) {
  ...
  try {
    ...
    if (typeof binary.version !== "string"
      ...
      throw new Error("R6_T4_PREFLIGHT_FAILED");
    }
    return Object.freeze({
      schemaVersion: "R6-T4-local-preflight-v1",
      ok: true,
      modelCall: false,
...
function verifyManifestInputs(root, manifest, requireProbes) {
  ...
  for (const task of manifest.tasks) {
    if (!ID.test(task.taskId)
      ...
      throw new Error("R6_T4_TASK_MANIFEST_INVALID");
    }
    ...
    if (sha256(prompt) !== task.promptHash
      ...
      throw new Error("R6_T4_TASK_INPUT_INVALID");
    }
  }
  if (armCount !== 10) throw new Error("R6_T4_ARM_COUNT_INVALID");
}
...
function collectFiles(root) {
  ...
  const visit = (directory) => {
    for (const entry of readdirSync(directory, { withFileTypes: true })
      ...
      if (entry.isSymbolicLink()) throw new Error("R6_T4_UNSAFE_NODE");
      if (entry.isDirectory()) visit(absolutePath);
      else if (entry.isFile()) {
        const size = statSync(absolutePath).size;
        if (size > MAX_FILE_BYTES) throw new Error("R6_T4_FILE_LIMIT");
        bytes += size;
        files.push({ absolutePath, relativePath });
      } else throw new Error("R6_T4_UNSAFE_NODE");
      if (files.length > MAX_FILES || bytes > MAX_TOTAL_BYTES) {
        throw new Error("R6_T4_EVIDENCE_LIMIT");
      }
    }
  };
  visit(root);
...
function requireExactChild(path, parent, mustBeNew) {
  const expectedParent = requireDirectory(parent, "R6_T4_PARENT_INVALID");
  const output = resolve(path);
  if (dirname(output) !== expectedParent
    || !ID.test(relative(expectedParent, output))
    || mustBeNew && existsSync(output)) {
    throw new Error("R6_T4_CHILD_PATH_INVALID");
  }
  if (!mustBeNew) requireDirectory(output, "R6_T4_CHILD_PATH_INVALID");
  return output;
}
...
function readRegular(path) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.size > MAX_FILE_BYTES) {
    throw new Error("R6_T4_FILE_INVALID");
  }
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === SCRIPT_PATH) {
  if (process.argv.length !== 6 || process.argv[2] !== "run") {
    throw new Error(
      "USAGE: node scripts/pilot/r6-t4.mjs run "
      + "<preregistration-root> <result-root> <authorization-json>",
    );
  }
  const result = await runR6RecoveryPilot({

...
import {
  ...
  R6_T13_VERDICT_RULE,
  validateR6T13ResponseText,
} from "./r6-attempt-four-plan.mjs";
import {
  executeR5BoundedProcess as executeR6T13BoundedProcess,
  installR5SignalForwarding as installR6T13SignalForwarding,
  verifyR5EvidenceManifest as verifyR6T13EvidenceManifest,
...
const R6_T10_RESULT_MANIFEST =
  "docs/test-evidence/R6-T10/20260730T195616-0800/RESULTS.sha256";
...
export const R6_T13_RUNTIME_SOURCE_PATHS = Object.freeze([
  "scripts/pilot/r6-t13.mjs",
  "scripts/pilot/r6-attempt-four-plan.mjs",
...
export function verifyR6T13OwnerAuthorization({
  authorization,
  preregistrationHash,
  dataScopeHash,
  terminationPolicy,
  ...
  if (![
    ...
    || authorization.schemaVersion !== "R6-T13-owner-authorization-v1"
    || authorization.taskId !== "R6-T13"
    || authorization.decisionKey !== "R6-RECOVERY"
    || authorization.decisionAttempt !== 4
    || authorization.supersedesAttempt !== "R6-T10"
    || authorization.authorized !== true
    || authorization.preregistrationHash !== preregistrationHash
    || authorization.destination !== R6_T13_DESTINATION
    || authorization.model !== R6_T13_MODEL
    || authorization.dataScopeHash !== dataScopeHash
    || authorization.outerEnvironmentHash !== outerEnvironmentHash
    || authorization.failureContractHash !== failureContractHash
    || authorization.failureCandidateHash !== failureCandidateHash
    || authorization.successfulArmCount
      !== R6_T13_TERMINATION_POLICY.requiredSuccessfulArms
    || authorization.maxProcessAttempts
      !== R6_T13_TERMINATION_POLICY.maxProcessAttempts
    || canonicalJson(authorization.terminationPolicy)
      !== canonicalJson(terminationPolicy)
    || canonicalJson(terminationPolicy)
      !== canonicalJson(R6_T13_TERMINATION_POLICY)
    ...
    throw new Error("R6_T13_OWNER_AUTHORIZATION_INVALID");
  }
  return Object.freeze({
    ...
    deadlineMs: R6_T13_TERMINATION_POLICY.deadlineMs,
    graceMs: R6_T13_TERMINATION_POLICY.graceMs,
    gracefulSignal: R6_T13_TERMINATION_POLICY.gracefulSignal,
    forceSignal: R6_T13_TERMINATION_POLICY.forceSignal,
    authorizationHash: canonicalSha256(authorization),
  });
}

export function verifyR6T13BoundSources(
  sourceBindings,
  { repoRoot = REPO_ROOT } = {},
) {
  const paths = Object.keys(sourceBindings ?? {}).sort();
  const expected = [...R6_T13_RUNTIME_SOURCE_PATHS].sort();
  if (canonicalJson(paths) !== canonicalJson(expected)) {
    throw new Error("R6_T13_SOURCE_BINDINGS_INVALID");
  }
  for (const path of expected) {
    const hash = sourceBindings[path];
    if (!HASH.test(hash ?? "")
      || sha256(readRegular(resolve(repoRoot, path))) !== hash) {
      throw new Error("R6_T13_BOUND_SOURCE_CHANGED");
    }
  }
  return Object.freeze({
    valid: true,
...
export function verifyR6T12Preregistration(
  rootPath,
  { repoRoot = REPO_ROOT, requireProbes = true } = {},
) {
  const root = requireDirectory(
    rootPath,
    "R6_T13_PREREGISTRATION_INVALID",
  );
  ...
  if (!HASH.test(expected) || actual !== expected) {
    throw new Error("R6_T13_PREREGISTRATION_CHANGED");
  }
  ...
  const failureContract = readJson(
    join(root, "inputs/failure-compatibility-contract.json"),
  );
  const failureCandidate = readJson(
    join(root, "inputs/failure-policy-candidate.json"),
  );
  const storedFailureValidation = readJson(
    join(root, "inputs/failure-policy-validation.json"),
  );
  const failureValidation = validateR6AttemptFourFailureCandidate({
    requirements: failureContract,
    candidate: failureCandidate,
  });
  const bindings = verifyR6T13BoundSources(plan.sourceBindings, { repoRoot });
  if (plan.schemaVersion !== "R6-T12-preregistered-recovery-plan-v1"
    || plan.decisionKey !== "R6-RECOVERY"
    || plan.decisionAttempt !== 4
    || plan.supersedesAttempt !== "R6-T10"
    ...
    || plan.model !== R6_T13_MODEL
    || plan.destination !== R6_T13_DESTINATION
    ...
    || plan.attemptThree?.taskId !== "R6-T10"
    || plan.attemptThree?.decisionAttempt !== 3
    || plan.attemptThree?.supersedesAttempt !== "R6-T7"
    || plan.attemptThree?.decision !== "adjust"
    || plan.attemptThree?.resultManifestHash
      !== R6_T10_RESULT_MANIFEST_HASH
    || plan.attemptThree?.verdictHash !== R6_T10_VERDICT_HASH
    || plan.attemptThree?.overwritten !== false
    || plan.r6T11ProofHash !== R6_T11_PROOF_HASH
    || plan.failureContractHash !== R6_T11_CONTRACT_HASH
    || plan.priorPreregistration?.taskId !== "R6-T9"
    || plan.priorPreregistration?.preregistrationHash !== R6_T9_HASH
    || plan.priorPreregistration?.transitivePriorDecisionChainBound !== true
    || plan.preparationSourceHash !== sha256(readRegular(
      resolve(repoRoot, "scripts/pilot/r6-t12.mjs"),
    ))
    || failureContract.contractHash !== plan.failureContractHash
    || plan.failureCandidateHash !== failureValidation.candidateHash
    || canonicalJson(storedFailureValidation)
      !== canonicalJson(failureValidation)
    || failureCandidate.compatibilityContractHash
      !== plan.failureContractHash
    || failureCandidate.externalExecutionAuthorized !== false
    ...
    || canonicalSha256(R6_T13_VERDICT_RULE) !== plan.verdictRuleHash
    || manifest.schemaVersion !== "R6-T12-task-manifest-v1"
    || manifest.decisionAttempt !== 4
    || manifest.supersedesAttempt !== "R6-T10"
    ...
    throw new Error("R6_T13_PREREGISTRATION_INVALID");
  }
...
export async function runR6AttemptFourPilot({
  preregistrationRoot,
  resultRoot,
  authorizationPath,
  ...
  if (typeof executeAttempt !== "function") {
    throw new Error("R6_T13_EXECUTOR_INVALID");
  }
  ...
  const preflight = runCurrentPreflight(
    preregRoot,
    verified.manifest,
    verified.failureValidation,
  );
  writeJson(join(outputRoot, "results/local-preflight.json"), preflight);

  const controller = new globalThis.AbortController();
  const removeSignalForwarding = installR6T13SignalForwarding({
    controller,
      ...
      try {
        ...
        for (const arm of task.armOrder) {
          ...
          for (let attemptNumber = 1; attemptNumber <= 2; attemptNumber += 1) {
            ...
            const sealed = sealR6AttemptFourEvidence({
              runId,
              armKey,
              attemptNumber,
            ...
            const retryPlan = planR6AttemptFourRetry({
              armKey,
  ...
  writeJson(join(outputRoot, "results/run-index.json"), {
    ...
    decisionAttempt: 4,
    supersedesAttempt: "R6-T10",
    attemptThreeResultManifestHash: R6_T10_RESULT_MANIFEST_HASH,
  ...
  return Object.freeze({
    ...
    remainingProcessAttemptBudget:
      R6_T13_TERMINATION_POLICY.maxProcessAttempts - processAttempts.length,
...
async function executeDefaultAttempt({
  ...
  const recorded = await executeR6T13BoundedProcess({
    invocation,
    outputRoot: runRoot,
    runId,
    ...
    evaluate({ finalResponse }) {
      ...
      const parsed = validateR6T13ResponseText(finalResponse);
      if (parsed === null) {
        return {
          status: "failed",
          valid: false,
          ...
          failureCodes: ["R6_T13_AUTHORITATIVE_RESPONSE_INVALID"],
        };
      }
      ...
      return {
        ...
        failureCodes: matches ? [] : ["R6_T13_WRONG_ATTRIBUTION"],
      };
    },
    signal,
  });
  ...
  const classification = classifyR6AttemptFour({
    ...
    evidenceSealed: recorded.evidenceSealed
      && verifyR6T13EvidenceManifest(runRoot),
...
function runCurrentPreflight(preregRoot, manifest, failurePolicy) {
  ...
  try {
    ...
    if (binary.version !== "codex-cli 0.144.5"
      || failurePolicy?.candidate?.externalExecutionAuthorized !== false
      ...
      throw new Error("R6_T13_PREFLIGHT_FAILED");
    }
    return Object.freeze({
      schemaVersion: "R6-T13-local-preflight-v1",
      ok: true,
      modelCall: false,
      ...
      failureCandidateHash: failurePolicy.candidateHash,
      permissionProfilePassed: true,
      authReadableToGeneratedCommands: false,
      workspaceWritable: false,
...
function verifyManifestInputs(root, manifest, requireProbes) {
  ...
  for (const task of manifest.tasks) {
    if (!ID.test(task.taskId)
      ...
      throw new Error("R6_T13_TASK_MANIFEST_INVALID");
    }
    ...
    if (sha256(prompt) !== task.promptHash
      ...
      throw new Error("R6_T13_TASK_INPUT_INVALID");
    }
  }
  if (armCount !== 10) throw new Error("R6_T13_ARM_COUNT_INVALID");
}

function verifyImmutableInputs(repoRoot) {
  if (sha256(readRegular(resolve(repoRoot, R6_T10_RESULT_MANIFEST)))
      !== R6_T10_RESULT_MANIFEST_HASH
    || sha256(readRegular(resolve(repoRoot, R6_T11_PROOF)))
      !== R6_T11_PROOF_HASH) {
    throw new Error("R6_T13_IMMUTABLE_INPUT_CHANGED");
  }
}
...
function collectFiles(root) {
  ...
  const visit = (directory) => {
    for (const entry of readdirSync(directory, { withFileTypes: true })
      ...
      if (entry.isSymbolicLink()) throw new Error("R6_T13_UNSAFE_NODE");
      if (entry.isDirectory()) visit(absolutePath);
      else if (entry.isFile()) {
        const size = statSync(absolutePath).size;
        if (size > MAX_FILE_BYTES) throw new Error("R6_T13_FILE_LIMIT");
        bytes += size;
        files.push({ absolutePath, relativePath });
      } else throw new Error("R6_T13_UNSAFE_NODE");
      if (files.length > MAX_FILES || bytes > MAX_TOTAL_BYTES) {
        throw new Error("R6_T13_EVIDENCE_LIMIT");
      }
    }
  };
  visit(root);
...
function requireExactChild(path, parent, mustBeNew) {
  const expectedParent = requireDirectory(parent, "R6_T13_PARENT_INVALID");
  const output = resolve(path);
  if (dirname(output) !== expectedParent
    || !ID.test(relative(expectedParent, output))
    || mustBeNew && existsSync(output)) {
    throw new Error("R6_T13_CHILD_PATH_INVALID");
  }
  if (!mustBeNew) requireDirectory(output, "R6_T13_CHILD_PATH_INVALID");
  return output;
}
...
function readRegular(path) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.size > MAX_FILE_BYTES) {
    throw new Error("R6_T13_FILE_INVALID");
  }
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === SCRIPT_PATH) {
  if (process.argv.length !== 6 || process.argv[2] !== "run") {
    throw new Error(
      "USAGE: node scripts/pilot/r6-t10.mjs run "
      + "<preregistration-root> <result-root> <authorization-json>",
    );
  }
  const result = await runR6AttemptFourPilot({

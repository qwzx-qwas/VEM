...
import {
  evaluateR6T10Recovery,
  R6_T9_DEADLINE_SELECTION,
  R6_T10_DESTINATION,
  R6_T10_MODEL,
  R6_T10_TERMINATION_POLICY,
  R6_T10_VERDICT_RULE,
  validateR6T10ResponseText,
} from "./r6-attempt-three-plan.mjs";
import {
  validateR6AttemptThreeDeadlineCandidate,
} from "./r6-dual-transport-horizon.mjs";
import {
  executeR5BoundedProcess as executeR6T10BoundedProcess,
  installR5SignalForwarding as installR6T10SignalForwarding,
  verifyR5EvidenceManifest as verifyR6T10EvidenceManifest,
...
const PREREGISTRATION_PARENT = join(REPO_ROOT, "docs/test-evidence/R6-T9");
const RESULT_PARENT = join(REPO_ROOT, "docs/test-evidence/R6-T10");
const R6_T6_HASH =
  "41ce5ab1a65437defdfcd86c0b4ec4db3e922af8a6c5e5642d44384ea910627e";
const R6_T7_RESULT_MANIFEST =
...
export const R6_T10_RUNTIME_SOURCE_PATHS = Object.freeze([
  "scripts/pilot/r6-t10.mjs",
  "scripts/pilot/r6-attempt-three-plan.mjs",
...
export function verifyR6T10OwnerAuthorization({
  authorization,
  preregistrationHash,
  dataScopeHash,
  terminationPolicy,
  ...
  if (![
    ...
    || authorization.schemaVersion !== "R6-T10-owner-authorization-v1"
    || authorization.taskId !== "R6-T10"
    || authorization.decisionKey !== "R6-RECOVERY"
    || authorization.decisionAttempt !== 3
    || authorization.supersedesAttempt !== "R6-T7"
    || authorization.authorized !== true
    || authorization.preregistrationHash !== preregistrationHash
    || authorization.destination !== R6_T10_DESTINATION
    || authorization.model !== R6_T10_MODEL
    ...
    || authorization.successfulArmCount
      !== R6_T10_TERMINATION_POLICY.requiredSuccessfulArms
    || authorization.maxProcessAttempts
      !== R6_T10_TERMINATION_POLICY.maxProcessAttempts
    || canonicalJson(authorization.terminationPolicy)
      !== canonicalJson(terminationPolicy)
    || canonicalJson(terminationPolicy)
      !== canonicalJson(R6_T10_TERMINATION_POLICY)
    ...
    throw new Error("R6_T10_OWNER_AUTHORIZATION_INVALID");
  }
  return Object.freeze({
    ...
    deadlineMs: R6_T10_TERMINATION_POLICY.deadlineMs,
    graceMs: R6_T10_TERMINATION_POLICY.graceMs,
    gracefulSignal: R6_T10_TERMINATION_POLICY.gracefulSignal,
    forceSignal: R6_T10_TERMINATION_POLICY.forceSignal,
    authorizationHash: canonicalSha256(authorization),
  });
}

export function verifyR6T10BoundSources(
  sourceBindings,
  { repoRoot = REPO_ROOT } = {},
) {
  const paths = Object.keys(sourceBindings ?? {}).sort();
  const expected = [...R6_T10_RUNTIME_SOURCE_PATHS].sort();
  if (canonicalJson(paths) !== canonicalJson(expected)) {
    throw new Error("R6_T10_SOURCE_BINDINGS_INVALID");
  }
  for (const path of expected) {
    const hash = sourceBindings[path];
    if (!HASH.test(hash ?? "")
      || sha256(readRegular(resolve(repoRoot, path))) !== hash) {
      throw new Error("R6_T10_BOUND_SOURCE_CHANGED");
    }
  }
  return Object.freeze({
    valid: true,
...
export function verifyR6T9Preregistration(
  rootPath,
  { repoRoot = REPO_ROOT, requireProbes = true } = {},
) {
  const root = requireDirectory(
    rootPath,
    "R6_T10_PREREGISTRATION_INVALID",
  );
  ...
  if (!HASH.test(expected) || actual !== expected) {
    throw new Error("R6_T10_PREREGISTRATION_CHANGED");
  }
  ...
  const bindings = verifyR6T10BoundSources(plan.sourceBindings, { repoRoot });
  if (plan.schemaVersion !== "R6-T9-preregistered-recovery-plan-v1"
    || plan.decisionKey !== "R6-RECOVERY"
    || plan.decisionAttempt !== 3
    || plan.supersedesAttempt !== "R6-T7"
    ...
    || plan.model !== R6_T10_MODEL
    || plan.destination !== R6_T10_DESTINATION
    ...
    || plan.attemptTwo?.taskId !== "R6-T7"
    || plan.attemptTwo?.decisionAttempt !== 2
    || plan.attemptTwo?.supersedesAttempt !== "R6-T4"
    || plan.attemptTwo?.decision !== "adjust"
    || plan.attemptTwo?.resultManifestHash
      !== R6_T7_RESULT_MANIFEST_HASH
    || plan.attemptTwo?.verdictHash !== R6_T7_VERDICT_HASH
    || plan.attemptTwo?.overwritten !== false
    || plan.r6T8ProofHash !== R6_T8_PROOF_HASH
    || plan.deadlineContractHash !== R6_T8_CONTRACT_HASH
    || plan.priorPreregistration?.taskId !== "R6-T6"
    || plan.priorPreregistration?.preregistrationHash !== R6_T6_HASH
    || plan.priorPreregistration?.transitivePriorDecisionChainBound !== true
    || plan.preparationSourceHash !== sha256(readRegular(
      resolve(repoRoot, "scripts/pilot/r6-t9.mjs"),
    ))
    ...
    || deadlineCandidate.terminalHorizonProvenance?.evidenceHash
      !== plan.versionBoundInstrumentationEvidenceHash
    || versionEvidence.provenanceKind
      !== deadlineCandidate.terminalHorizonProvenance?.kind
    || versionEvidence.codexVersion !== deadlineCandidate.codexVersion
    || canonicalJson(versionEvidence.deadlineSelection)
      !== canonicalJson(R6_T9_DEADLINE_SELECTION)
    ...
    || versionEvidence.r6T8ProofHash !== R6_T8_PROOF_HASH
    ...
    || deadlineCandidate.providerTerminalHorizonMs
      !== R6_T9_DEADLINE_SELECTION.providerTerminalHorizonMs
    || deadlineCandidate.terminalObservationMarginMs
      !== deadlineContract.minimumTerminalObservationMarginMs
    || canonicalSha256(R6_T10_VERDICT_RULE) !== plan.verdictRuleHash
    || manifest.schemaVersion !== "R6-T9-task-manifest-v1"
    || manifest.decisionAttempt !== 3
    || manifest.supersedesAttempt !== "R6-T7"
    ...
    throw new Error("R6_T10_PREREGISTRATION_INVALID");
  }
...
export async function runR6AttemptThreePilot({
  preregistrationRoot,
  resultRoot,
  authorizationPath,
  ...
  const verified = verifyR6T9Preregistration(preregRoot);
  const authorization = readJson(resolve(authorizationPath));
  const authorizationEvidence = verifyR6T10OwnerAuthorization({
    authorization,
  ...
  if (typeof executeAttempt !== "function") {
    throw new Error("R6_T10_EXECUTOR_INVALID");
  }
  ...
  const removeSignalForwarding = installR6T10SignalForwarding({
    controller,
  ...
  const verdict = evaluateR6T10Recovery({
    arms,
    processAttempts,
  ...
  writeJson(join(outputRoot, "results/run-index.json"), {
    ...
    decisionAttempt: 3,
    supersedesAttempt: "R6-T7",
    attemptTwoResultManifestHash: R6_T7_RESULT_MANIFEST_HASH,
  ...
  return Object.freeze({
    ...
    remainingProcessAttemptBudget:
      R6_T10_TERMINATION_POLICY.maxProcessAttempts - processAttempts.length,
...
async function executeDefaultAttempt({
  ...
  const recorded = await executeR6T10BoundedProcess({
    invocation,
    outputRoot: runRoot,
    runId,
    ...
    evaluate({ finalResponse }) {
      ...
      const parsed = validateR6T10ResponseText(finalResponse);
      if (parsed === null) {
        return {
          status: "failed",
          valid: false,
          ...
          failureCodes: ["R6_T10_AUTHORITATIVE_RESPONSE_INVALID"],
        };
      }
      ...
      return {
        ...
        failureCodes: matches ? [] : ["R6_T10_WRONG_ATTRIBUTION"],
      };
    },
    signal,
  });
  ...
  const classification = classifyR6Attempt({
    ...
    evidenceSealed: recorded.evidenceSealed
      && verifyR6T10EvidenceManifest(runRoot),
  });
...
function runCurrentPreflight(preregRoot, manifest, deadlineCandidate) {
  ...
  try {
    ...
    if (binary.version !== deadlineCandidate.codexVersion
      ...
      throw new Error("R6_T10_PREFLIGHT_FAILED");
    }
    return Object.freeze({
      schemaVersion: "R6-T10-local-preflight-v1",
      ok: true,
      modelCall: false,
...
function verifyManifestInputs(root, manifest, requireProbes) {
  ...
  for (const task of manifest.tasks) {
    if (!ID.test(task.taskId)
      ...
      throw new Error("R6_T10_TASK_MANIFEST_INVALID");
    }
    ...
    if (sha256(prompt) !== task.promptHash
      ...
      throw new Error("R6_T10_TASK_INPUT_INVALID");
    }
  }
  if (armCount !== 10) throw new Error("R6_T10_ARM_COUNT_INVALID");
}

function verifyImmutableInputs(repoRoot) {
  if (sha256(readRegular(resolve(repoRoot, R6_T7_RESULT_MANIFEST)))
      !== R6_T7_RESULT_MANIFEST_HASH
    || sha256(readRegular(resolve(repoRoot, R6_T8_PROOF)))
      !== R6_T8_PROOF_HASH) {
    throw new Error("R6_T10_IMMUTABLE_INPUT_CHANGED");
  }
}
...
function collectFiles(root) {
  ...
  const visit = (directory) => {
    for (const entry of readdirSync(directory, { withFileTypes: true })
      ...
      if (entry.isSymbolicLink()) throw new Error("R6_T10_UNSAFE_NODE");
      if (entry.isDirectory()) visit(absolutePath);
      else if (entry.isFile()) {
        const size = statSync(absolutePath).size;
        if (size > MAX_FILE_BYTES) throw new Error("R6_T10_FILE_LIMIT");
        bytes += size;
        files.push({ absolutePath, relativePath });
      } else throw new Error("R6_T10_UNSAFE_NODE");
      if (files.length > MAX_FILES || bytes > MAX_TOTAL_BYTES) {
        throw new Error("R6_T10_EVIDENCE_LIMIT");
      }
    }
  };
  visit(root);
...
function requireExactChild(path, parent, mustBeNew) {
  const expectedParent = requireDirectory(parent, "R6_T10_PARENT_INVALID");
  const output = resolve(path);
  if (dirname(output) !== expectedParent
    || !ID.test(relative(expectedParent, output))
    || mustBeNew && existsSync(output)) {
    throw new Error("R6_T10_CHILD_PATH_INVALID");
  }
  if (!mustBeNew) requireDirectory(output, "R6_T10_CHILD_PATH_INVALID");
  return output;
}
...
function readRegular(path) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.size > MAX_FILE_BYTES) {
    throw new Error("R6_T10_FILE_INVALID");
  }
...
if (process.argv[1] !== undefined && resolve(process.argv[1]) === SCRIPT_PATH) {
  ...
  const result = await runR6AttemptThreePilot({

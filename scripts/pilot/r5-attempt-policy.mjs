...
import { R5_TERMINATION_POLICY } from "./r5-recovery-plan.mjs";
...
export function classifyR5Attempt({
  run,
  termination,
  finalResponse,
  boundary,
  ...
  if (!isRecord(run)
    ...
    throw new Error("R5_ATTEMPT_CLASSIFICATION_INPUT_INVALID");
  }
  ...
  return deepFreeze({
    schemaVersion: "R5-T3-attempt-classification-v1",
    kind,
    retryable,
    evidence: {
      sealedBoundary,
...
export function sealR5AttemptEvidence({
  runId,
  armKey,
  attemptNumber,
  classification,
  ...
  if (!ID.test(runId)
    ...
    || attemptNumber < 1
    || attemptNumber > R5_TERMINATION_POLICY.maxRetriesPerArm + 1
    || classification?.schemaVersion
      !== "R5-T3-attempt-classification-v1"
    ...
    throw new Error("R5_ATTEMPT_EVIDENCE_INVALID");
  }
  const record = {
    schemaVersion: "R5-T3-sealed-attempt-v1",
    runId,
    armKey,
    attemptNumber,
    classification,
...
export function planR5Retry({
  ...
  if (!ID.test(armKey)
    ...
    || attempts.length > R5_TERMINATION_POLICY.maxRetriesPerArm + 1
    || !Number.isSafeInteger(batchProcessAttemptCount)
    || batchProcessAttemptCount < attempts.length
    || batchProcessAttemptCount > R5_TERMINATION_POLICY.maxProcessAttempts
    || attempts.some((attempt, index) => (
      attempt?.schemaVersion !== "R5-T3-sealed-attempt-v1"
    ...
    || new Set(attempts.map((attempt) => attempt.runId)).size
      !== attempts.length) {
    throw new Error("R5_RETRY_STATE_INVALID");
  }
  ...
  if (attempts.length > R5_TERMINATION_POLICY.maxRetriesPerArm) {
    return decision("stop", "retry-exhausted", null, historyHash);
  }
  if (batchProcessAttemptCount >= R5_TERMINATION_POLICY.maxProcessAttempts) {
...
function decision(action, reason, nextAttemptNumber, attemptHistoryHash) {
  return deepFreeze({
    schemaVersion: "R5-T3-retry-decision-v1",
    action,
    reason,
    nextAttemptNumber,
    ...
    runnerTerminationRetryable:
      R5_TERMINATION_POLICY.runnerTerminationRetryable,
    attemptHistoryHash,
  });
}
